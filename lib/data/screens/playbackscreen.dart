...
class _PlaybackPageState1 extends State<PlaybackPage1> {
  ...
  void playRoute() async {
    ...
    interval(new Duration(milliseconds: playbackTime), (timer) {
      ...
      if (routeList.length - 1 == _sliderValue.toInt()) {
        ...
        _markers.add(
          Marker(
            markerId: MarkerId(routeList[_sliderValue.toInt()].id.toString()),
            position: LatLng(
                routeList[_sliderValue.toInt()].latitude!.toDouble(),
                routeList[_sliderValue.toInt()]
                    .longitude!
                    .toDouble()), // updated position
    ...
    if (routeList.length != _sliderValue.toInt()) {
      ...
      _markers.add(
        Marker(
          ...
          position: LatLng(
              routeList[_sliderValue.toInt()].latitude as double,
              routeList[_sliderValue.toInt()].longitude
                  as double), // updated position
  ...
  Widget build(BuildContext context) {
    ...
    choices = <Choice>[
      Choice(title: 'slow', icon: Icons.directions_car),
      Choice(title: 'medium', icon: Icons.directions_bike),
      Choice(title: 'fast', icon: Icons.directions_boat),
    ];
...
//            ),
                      ...
                      RadioListTile(
                        ...
                        onChanged: (String? value) {
                          ...
                          StaticVarMethod.todate =
                              DateFormat('yyyy-MM-dd').format(todaydate);
                          StaticVarMethod.totime = DateFormat('HH:mm').format(
                              DateTime(
                                  DateTime.now().year,
                                  DateTime.now().month,
                                  DateTime.now().day,
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(_fromdate.toString(),
                            style: GlobalStyle.deliveryPrice),
                      ],
                    ),
                  ),
                ),
  ...
  Widget playBackControls() {
    ...
    if (routeList.length > _sliderValue.toInt()) {
      fUpdateTime =
          formatTime(routeList[_sliderValue.toInt()].rawTime.toString());
                ...
                child: Row(
                  children: <Widget>[
                    Container(
                      padding: EdgeInsets.only(left: 5.0),
                      child:
                          Icon(Icons.av_timer, color: Colors.black, size: 20.0),
                    ),
                    Container(
                      padding: EdgeInsets.only(left: 5.0),
                      child: Text('Update' + ": " + fUpdateTime),
                    ),
                  ],
                ),
              ),
