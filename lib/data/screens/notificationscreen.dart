...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  initState() {
    ...
    (StaticVarMethod.notificationback)
        ? getnotiList()
        : StaticVarMethod.notificationback == true;
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                                ...
                                child: Column(
                                  children: [
                                    RichText(
                                      ...
                                      text: TextSpan(
                                        text:
                                            'Em algum momento, você pode não receber notificações devido à cobertura do GPS, erro de rede, bateria, problemas de tensão e veículos não suportados.',
                                        style: TextStyle(
                                          fontSize: 14,
                                          color: Colors.black,
      ...
      //   elevation: 0,
      //   leading: IconButton(
      //     icon: Icon(Icons.arrow_back, color: (StaticVarMethod.notificationback)?Colors.white:Colors.black),
      ...
      // //   color: (StaticVarMethod.notificationback)?Colors.white:Colors.black,
      // // ),
      ...
      // ),
      body: StaticVarMethod.eventList.length > 0
          ? listView() //listView()
          : (_isLoading)
              ? Center(
            ...
            tiles: eventList.map((item) => ListTile(
                  leading: CircleAvatar(
                    ...
                    child: CachedNetworkImage(
                      imageUrl: 'http://116.58.56.123:8008/Content/EmpImg/' +
                          item.id.toString() +
                          '.jpg',
  ...
  Widget listView() {
    return ListView.builder(
        itemCount: StaticVarMethod.eventList.length,
  ...
  Widget listViewItems(int index) {
    return GestureDetector(
        onTap: () {
          StaticVarMethod.lat =
              StaticVarMethod.eventList[index].latitude as double;
          StaticVarMethod.lng =
              StaticVarMethod.eventList[index].longitude as double;
          StaticVarMethod.deviceName =
              StaticVarMethod.eventList[index].deviceName.toString();
          StaticVarMethod.type =
              StaticVarMethod.eventList[index].type.toString();
          StaticVarMethod.message =
              StaticVarMethod.eventList[index].message.toString();
          StaticVarMethod.time =
              StaticVarMethod.eventList[index].time.toString();
          StaticVarMethod.speed =
              StaticVarMethod.eventList[index].speed.toString();
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => notificationmapscreen()),
          );
        },
        child: Container(
            margin: EdgeInsets.fromLTRB(6, 6, 6, 0),
...
/*  Widget listViewItems(int index){
  ...
  Widget prefixIcon(int index) {
    var empid = StaticVarMethod.eventList[index].id;
    return /*Container(
  ...
  Widget message(int index) {
    double textsize = 12;
    var msg = StaticVarMethod.eventList[index].message.toString();
      ...
      child: RichText(
        ...
        text: TextSpan(
            text: 'Your Device ' +
                StaticVarMethod.eventList[index].deviceName.toString() +
                ' generate events of (' +
                StaticVarMethod.eventList[index].name.toString() +
                ')',
            style: TextStyle(
              fontSize: textsize,
              color: Colors.grey.shade700,
