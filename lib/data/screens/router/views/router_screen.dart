...
class _RouterScreenState extends State<RouterScreen> {
  ...
  // Histórico de rotas
  List<SavedRouteData> _routeHistory = [];
  ...
  // Vincular veículo à rota
  void _linkVehicleToRoute(deviceItems vehicle) {
    setState(() {
      _linkedVehicle = vehicle;
    });
    
    // Se o veículo tem coordenadas, definir como destino
    if (vehicle.lat != null && vehicle.lng != null) {
      _destinationController.text = vehicle.name ?? 'Veículo ${vehicle.id}';
      _destinationCoords = LatLng(
        vehicle.lat!.toDouble(), 
        vehicle.lng!.toDouble()
      );
      _calculateRoute();
    }
    
    Fluttertoast.showToast(
      msg: 'Veículo ${vehicle.name} vinculado à rota',
  ...
  Future<void> _calculateRoute() async {
    ...
    try {
      ...
      // Consumo fixo baseado no tipo de veículo
      ...
      final savedRoute = SavedRouteData(
        ...
        linkedVehicleId: _linkedVehicle?.id,
      ...
      setState(() {
        ...
        _isLoadingRoute = false;
        _routeHistory.insert(0, savedRoute);
  ...
  FuelConsumption _calculateFuelConsumption(double distanceKm, [double? customConsumption]) {
    final consumptionKmL = customConsumption ?? (_selectedVehicleType == VehicleType.car ? 12.0 : 6.0);
...
_Dados atualizados com tráfego em tempo real_
  ...
  Color _getVehicleStatusColor(deviceItems vehicle) {
    if (vehicle.online == 'online') {
      if (vehicle.speed != null && vehicle.speed! > 0) {
        return Colors.green; // Online e em movimento
      } else {
        return Colors.yellow; // Online mas parado
      }
    } else {
  ...
  Future<void> _loadRouteHistory() async {
    try {
      ...
      final historyJson = prefs.getStringList('router_history') ?? [];
      
      setState(() {
        _routeHistory = historyJson
            .map((json) => SavedRouteData.fromJson(jsonDecode(json)))
            .toList();
      });
      
      print('✅ Carregadas ${_routeHistory.length} rotas do histórico');
    } catch (e) {
  ...
  Future<void> _saveRouteHistory() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final historyJson = _routeHistory
          .map((route) => jsonEncode(route.toJson()))
          .toList();
      
      await prefs.setStringList('router_history', historyJson);
      print('✅ Histórico salvo (${_routeHistory.length} rotas)');
    } catch (e) {
      print('❌ Erro ao salvar histórico: $e');
    }
  }

  // Mostrar modal com detalhes da rota
  void _showRouteDetailsModal(SavedRouteData savedRoute) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => _buildRouteDetailsModal(savedRoute),
    );
  }
        ...
        return Scaffold(
            ...
            body: Column(
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                                  ...
                                  Expanded(
                                    child: GestureDetector(
                                      onTap: () {
                                        ...
                                        if (_routeDetails != null) {
                                          _calculateRoute(); // Recalcular se já tem rota
                                        }
                                      },
                                      child: Container(
                      ...
                      // Seletor de veículo para vincular à rota
                      Container(
                        padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                        decoration: BoxDecoration(
                          color: Colors.grey[50],
                        ...
                        child: Consumer<ObjectStore>(
                          builder: (context, objectStore, _) {
                            final vehicles = objectStore.objects;
                            ...
                            return Column(
                              ...
                              children: [
                                Row(
                                  children: [
                                    Icon(Icons.directions_car_outlined, color: Colors.grey[600], size: 16),
                                    SizedBox(width: 8),
                                    Text(
                                      'Vincular veículo:', 
                                      style: TextStyle(fontSize: 12, color: Colors.grey[600]),
                                ...
                                SizedBox(
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      final isSelected = _linkedVehicle?.id == vehicle.id;
                                      
                                      return Container(
                                        margin: EdgeInsets.only(right: 8),
                                        child: GestureDetector(
                                          onTap: () => _linkVehicleToRoute(vehicle),
                                          child: Container(
                                            ...
                                            child: Row(
                                              ...
                                              children: [
                                                Container(
                                                  ...
                                                  height: 8,
                                                  decoration: BoxDecoration(
                                                    color: _getVehicleStatusColor(vehicle),
                                                    shape: BoxShape.circle,
                                                  ),
                                                ),
                                                SizedBox(width: 6),
                                                Text(
                                                  vehicle.name ?? 'V${vehicle.id}',
                                                  style: TextStyle(
                                                    fontSize: 11,
                                                    fontWeight: FontWeight.w500,
                                                    color: isSelected 
                ...
                // Conteúdo principal - Mapa e Detalhes da Rota
                Expanded(
                  child: _showRouteDetails 
                      ? _buildMapWithRouteDetails(colorProvider)
                      : _buildEmptyStateWithMap(colorProvider),
                ),
  ...
  // Mapa com detalhes da rota
  Widget _buildMapWithRouteDetails(ColorProvider colorProvider) {
    return Column(
      children: [
        // Mapa (50% da tela)
        Expanded(
        ...
        // Detalhes da rota atual (25% da tela)
        Container(
          height: 120,
          decoration: BoxDecoration(
            color: Colors.white,
          ...
          child: _buildCurrentRouteDetails(colorProvider),
        ),
        
        // Histórico de rotas (25% da tela)
        Expanded(
          flex: 1,
          child: Container(
            decoration: BoxDecoration(
            ...
            child: _buildRouteHistorySection(colorProvider),
          ),
        ),
      ],
    );
  ...
  Widget _buildCurrentRouteDetails(ColorProvider colorProvider) {
    if (_routeDetails == null) {
      return Center(
        child: Text(
    ...
    final route = _routeDetails!;
    final vehicleDuration = _selectedVehicleType == VehicleType.car 
        ? route.carDuration 
        : route.truckDuration;
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${route.distanceText} • ${_formatDuration(vehicleDuration)}',
                      style: TextStyle(
                        fontSize: 18,
                        fontWeight: FontWeight.bold,
                        color: colorProvider.primaryColor,
  ...
  // Seção de histórico de rotas
  Widget _buildRouteHistorySection(ColorProvider colorProvider) {
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 8),
              Text(
                'Histórico (${_routeHistory.length})',
                style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
                  color: colorProvider.primaryColor,
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return _buildHistoryCard(colorProvider, _routeHistory[index]);
            },
          ),
        ),
      ],
  ...
  // Card do histórico
  Widget _buildHistoryCard(ColorProvider colorProvider, SavedRouteData savedRoute) {
    ...
    return Container(
      ...
      child: InkWell(
        onTap: () => _showRouteDetailsModal(savedRoute),
        borderRadius: BorderRadius.circular(12),
        child: Padding(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (savedRoute.linkedVehicleName != null) ...[
                      SizedBox(height: 4),
                      Row(
                        children: [
                          Icon(Icons.directions_car, size: 10, color: Colors.blue[600]),
                          SizedBox(width: 4),
                          Text(
                            savedRoute.linkedVehicleName!,
                            style: TextStyle(
                              fontSize: 10,
  ...
  // Modal com detalhes completos da rota
  Widget _buildRouteDetailsModal(SavedRouteData savedRoute) {
    final route = savedRoute.routeDetails;
    final vehicleDuration = savedRoute.vehicleType == VehicleType.car 
        ? route.carDuration 
        : route.truckDuration;
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(width: 8),
                              Text(
                                'Veículo vinculado: ${savedRoute.linkedVehicleName}',
                                style: TextStyle(
                                  fontSize: 14,
                                  color: Colors.blue[700],
                                  fontWeight: FontWeight.w500,
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text(
                              'Criada em: ${_formatDateTime(savedRoute.calculatedAt)}',
                              style: TextStyle(
                                fontSize: 14,
...
// Classe para armazenar dados de rota salva
class SavedRouteData {
  ...
  final String? linkedVehicleName;
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'routeDetails': {
        ...
        'carDuration': routeDetails.carDuration,
        'truckDuration': routeDetails.truckDuration,
        'originAddress': routeDetails.originAddress,
        'destinationAddress': routeDetails.destinationAddress,
        'hasTolls': routeDetails.hasTolls,
        'estimatedTollCost': routeDetails.estimatedTollCost,
        'fuelConsumption': {
          'estimatedCost': routeDetails.fuelConsumption.estimatedCost,
        },
        'polylinePoints': routeDetails.polylinePoints.map((p) => {
          'lat': p.latitude,
          'lng': p.longitude,
        }).toList(),
        'bounds': routeDetails.bounds != null ? {
          'southwest': {
            'lat': routeDetails.bounds!.southwest.latitude,
            'lng': routeDetails.bounds!.southwest.longitude,
          },
          'northeast': {
            'lat': routeDetails.bounds!.northeast.latitude,
            'lng': routeDetails.bounds!.northeast.longitude,
          },
        } : null,
      },
      'originCoords': {
  ...
  factory SavedRouteData.fromJson(Map<String, dynamic> json) {
    final routeDetailsJson = json['routeDetails'] as Map<String, dynamic>;
    
    return SavedRouteData(
      id: json['id'],
      routeDetails: RouteDetails(
        distanceText: routeDetailsJson['distanceText'],
        distanceKm: routeDetailsJson['distanceKm'],
        carDuration: routeDetailsJson['carDuration'],
        truckDuration: routeDetailsJson['truckDuration'],
        polylinePoints: (routeDetailsJson['polylinePoints'] as List).map((p) => 
          LatLng(p['lat'], p['lng'])
        ).toList(),
        bounds: routeDetailsJson['bounds'] != null 
          ? LatLngBounds(
              southwest: LatLng(
                routeDetailsJson['bounds']['southwest']['lat'],
                routeDetailsJson['bounds']['southwest']['lng'],
              ),
              northeast: LatLng(
                routeDetailsJson['bounds']['northeast']['lat'],
                routeDetailsJson['bounds']['northeast']['lng'],
              ),
            )
          : null,
        originAddress: routeDetailsJson['originAddress'],
        destinationAddress: routeDetailsJson['destinationAddress'],
        trafficLevel: TrafficLevel.unknown,
        trafficDescription: '',
        delaySeconds: 0,
        hasTolls: routeDetailsJson['hasTolls'] ?? false,
        estimatedTollCost: routeDetailsJson['estimatedTollCost'] ?? 0.0,
        tollCount: 0,
        tollRoads: [],
        fuelConsumption: FuelConsumption(
          distanceKm: routeDetailsJson['distanceKm'],
          consumptionKmL: 12.0,
          litersNeeded: 0,
          estimatedCost: routeDetailsJson['fuelConsumption']['estimatedCost'] ?? 0.0,
        ),
        routeSummary: '',
        warnings: [],
        routeFeatures: {},
        alternativeRoutes: [],
        totalRoutesFound: 1,
        optimizedDepartureTimes: [],
        hasRestrictions: false,
        detailedSteps: [],
      ),
      originCoords: LatLng(
        json['originCoords']['lat'],
        json['originCoords']['lng'],
      ),
      destinationCoords: LatLng(
        json['destinationCoords']['lat'],
        json['destinationCoords']['lng'],
      ),
      vehicleType: json['vehicleType'] == 'VehicleType.car' 
          ? VehicleType.car 
          : VehicleType.truck,
      linkedVehicleId: json['linkedVehicleId'],
