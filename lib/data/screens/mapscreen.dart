...
class _mapscreen extends State<mapscreen> {
  ...
  Future<ui.Image> _loadImage(List<int> img) async {
    ...
    final Uint8List uint8ListImg = Uint8List.fromList(img);

    // Agora passa o Uint8List para IMG.decodeImage
    final IMG.Image image = IMG.decodeImage(uint8ListImg)!;
    final IMG.Image resized =
    IMG.copyResize(image, width: _iconFacebookSize.toInt());
  ...
  Future<BitmapDescriptor> _createImageLabel(
      {String label = 'label',
        required ui.Image imageIcon,
        double fontSize = 30}) async {
    ui.PictureRecorder recorder = ui.PictureRecorder();
    final canvas = Canvas(recorder);
    CustomImageAssets imageLabeling = CustomImageAssets(
        imageLabel: label,
        imageData: imageIcon,
        fontSize: fontSize,
  ...
  void _addMarker(double lat, double lng) {
    ...
    _createImageLabel(label: 'Facebook Labeling', imageIcon: _iconFacebook)
        ...
        _marker = Marker(
          ...
          onTap: () {
            Fluttertoast.showToast(
                msg: 'Click marker', toastLength: Toast.LENGTH_SHORT);
          },
          icon: customIcon,
        );
      });
